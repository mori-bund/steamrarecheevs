...
def get_owned_games(steamid):
	...
	"""
	owned_games = api.IPlayerService.GetOwnedGames(steamid=steamid, include_appinfo=True, include_played_free_games=True, appids_filter=False, include_free_sub=False, language='en', include_extended_appinfo=False)
...
def get_game_achievement_data(appid):
	...
	"""
	try:
		achievement_data = api.ISteamUserStats.GetGlobalAchievementPercentagesForApp(gameid=appid)['achievementpercentages']['achievements']
		if achievement_data == []:
			return None
		else:
...
def player_has_completed(steamid, appid):
	...
	"""
	try:
		player_data = api.ISteamUserStats.GetPlayerAchievements(steamid=steamid,appid=appid)
	except requests.exceptions.HTTPError as e:
		return None
...
def scrape_steam_data(steamid, existing_appids, owned_games, progress_bar):
	...
	for game in owned_games:
		...
		scraped_data.append([appid, game_name, rarest_achievement_percentage, has_completed])
