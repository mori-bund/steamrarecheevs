...
def main():
    ...
    parser = ArgumentParser(
        description='Steam Library Scraper to find rarest achievements')
    ...
    num_args_provided = sum(
        [1 for arg in [args.steamid, args.vanity, args.update_no_achievements] if arg])
    ...
    try:
        scraped_data, no_achievements = scrape_steam_data(
            steamid, existing_appids, owned_games, progress_bar)
    except Exception as e:
